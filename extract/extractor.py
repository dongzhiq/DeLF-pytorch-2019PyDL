...
from genericdataset import ImagesFromList
__DEBUG__ = False
...
class FeatureExtractor():
    ...
    def __extract_delf_feature__(self, x, filename, mode='pca'):
        ...
        try:
            ...
            'filename':filename,
            'location_np_list':b,
            'descriptor_np_list':b,
            'feature_scale_np_list':b,
            'attention_score_np_list':b,
            'attention_np_list':b
             }
            return data
    ...
    def __save_delf_features_to_file__(self,
        ...
        '''
        s=pickle.dumps(data,protocol=2)
        f=open(filename,'wb')
        f.write(s)
        f.close
    ...
    def extract1(self, images, output_path):
        '''extract features from single image without batch process.
        '''
        assert self.mode.lower() in ['pca', 'delf']
        batch_timer = AverageMeter()
        ...
        since = time.time()
        self.dataloader = torch.utils.data.DataLoader(
            ImagesFromList(root='', images=images, transform=transforms.ToTensor()),
...
if __name__ == "__main__":
    ...
    USE_PCA = True
    PCA_DIMS = 40
    SCALE_LIST = [0.25,0.3535,0.5, 0.7071, 1.0,1.4142,2.0] #0.3535,1.4142,2.0
    ...
    LOAD_FROM = '/home/yangyc/Delf-pytorch/DeLF-pytorch-master/DeLF-pytorch-master/train/repo/devel/keypoint/ckpt/fix.pth.tar'
    PCA_PARAMETERS_PATH = '/home/yangyc/Delf-pytorch/DeLF-pytorch-master/DeLF-pytorch-master/extract/pca.h5'
    ...
    if MODE.lower() in ['pca']:
        OUTPUT_PATH = 'dummy'
        INPUT_PATH = '/home/yangyc/revisitop-master/data/datasets/roxford5k/'
        extractor.extract(INPUT_PATH, OUTPUT_PATH)
    
    elif MODE.lower() in ['delf']:
        # query
        INPUT_PATH = '/home/yangyc/revisitop-master/data/datasets/'
        OUTPUT_PATH = 'ox5kdelf-3'
        OUTPUT_PATH1 = 'ox5kdelfquery-3'
        cfg = configdataset(dataset,INPUT_PATH)
        images = [cfg['im_fname'](cfg,i) for i in range(cfg['n'])]
        qimages = [cfg['qim_fname'](cfg,i) for i in range(cfg['nq'])]
        
        extractor.extract1(qimages, OUTPUT_PATH1)
        extractor.extract1(images, OUTPUT_PATH)
        # index
